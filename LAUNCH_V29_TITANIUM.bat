...
:: ║                                                                  ║
:: ║  v29 FIXES over v28 launcher:                                   ║
:: ║  • ASR (Attack Surface Reduction) exclusions — separate from    ║
:: ║    ExclusionPath (v28 bug: only set ExclusionPath, not ASR)     ║
:: ║  • Try --only-binary pip installs first (no meson/ninja/ASR)    ║
:: ║  • Rotates crash_fault.log/dp_stderr.txt (keeps last 3)         ║
:: ║  • Python 3.13 support + smarter discovery                      ║
:: ║  • Expanded C2 block list (Kimwolf, BadBox2, AISURU, Mozi)      ║
:: ║  • PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD=1 prevents ASR trigger      ║
:: ║  • Clears stale tk_callback_errors.txt before launch            ║
:: ║  • Virtual memory check warns if < 2GB free                     ║
:: ║  • Targets downpour_v29_titanium.py specifically                ║
