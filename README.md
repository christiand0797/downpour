...
<p align="center">
  <img src="https://img.shields.io/badge/version-v29.48%20Titanium-blue?style=for-the-badge" />
...
**v29.44d**: Persistence watchers (`persistence_watchers.py`) — the final 3 audit blind spots: registry auto-start keys (Run/RunOnce/Winlogon baseline+diff, persistent baselines catch offline writes — T1060), DLL planting in writable PATH dirs (19 shadowed system DLLs — T1574.001), and kernel-driver monitoring with BYOVD blocklist (gdrv/dbutil/RTCore64... — T1068). Wired into security monitors with `[PERSIST]` alerting. 14 new tests — 250/250 pass.
...
**v29.43e**: Memory-DoS fix in the quarantine service — large files are stream-encrypted/restored in 64KiB AES-GCM chunks (DQS2 format, per-chunk nonces, constant memory); old formats remain restorable. TASK-010 deferred with honest rationale (yara-python is CPU-only; RTX 3050 present, gap is software). 191/191 tests.

**v29.43d**: Final quarantine unification — the Threats-tab GUI producer (`_threats_quarantine_selected`) migrated to the quarantine service; **every quarantine path now shares one format, one manifest scheme, one DB**, and GUI-quarantined files are restorable via `_remediation_revert` for the first time. Fixed two bugs in the newly added TASK-007 tests (missing `import socket`; `Allow_HTTP`/`Allow_HTTPS` prefix collision). Concurrent work merged & verified (corroboration gate, sensor_hub). 187/187 tests.

**v29.43c**: TASK-016 residuals closed — collision fix (v2 `safe_name` now timestamp-suffixed on clash; re-quarantining the same file no longer overwrites the prior copy) and `migrate_legacy_entries()` (ingests v1 AES/XOR sidecar entries hash-verified + plain `.locked`/`.quar` moves into the v2 service, legacy artifacts renamed `*.migrated`), wired into `system_cleanup`. Residual: migrate the GUI `.quar` producer. 4 new tests — 175/175 pass.

**v29.43b**: Quarantine service v2 (concurrent rewrite) merged — all four quarantine call sites rewired to the new SQLite-tracked `QuarantineService` API (entry-based restores, per-file security-descriptor preservation, write-ahead manifests), quarantine tests rewritten for it. TASK-007 critical-path tests added (kill-switch fail-closed behavior, netsh parameter-injection fix in `_block_ip`, revert wiring). Migration gaps documented (legacy formats, GUI `.quar` producer, possible v2 collision). 171/171 tests.

**v29.42z**: TASK-016 follow-ups shipped — `reconcile_quarantine()` (boot-time manifest/content cross-check with `restored=2` evidence-lost marking) and `harden_quarantine_dacl()` (icacls, SYSTEM+Administrators only via language-neutral SIDs, elevated-only), wired into `system_cleanup` (`--reconcile` + pre-restore) and a new `quarantine_core` CLI. 4 new tests — 158/158 pass.

**v29.42y**: TASK-013 (CRITICAL) + TASK-015 (HIGH) closed. Feed ingestion: both fetch paths are now **HTTPS-only** with per-fetch `FEED-INTEGRITY` sha256 audit lines (the plain-HTTP fallback was a content-swap window; kimwolf's "auto-block" turned out to be dead code — audit corrected). Allowlists: new `trust_check.py` binds system-process trust to **exact name + image path under %SystemRoot% + valid Authenticode signature** (WinVerifyTrust with a signed-HRESULT mask fix, plus a PSModulePath-safe `Get-AuthenticodeSignature` fallback, cached); `behavior_scanner`'s substring safe-skip (`safe in name`) and the "KNOWN SAFE by name" display are fixed. 8 new tests — 154/154 pass.

**v29.42x**: TASK-016 (HIGH) — quarantine unified. New `quarantine_core.py`: AES-256-GCM (DPAPI-protected key, XOR fallback), write-ahead manifest + self-verified encrypted copy **before** the original is deleted, collision-safe naming, and tamper-refusing hash-verified restore (temp file + `os.replace`). All real quarantine paths now use it: `mitigate()` (old plain move could silently overwrite `{name}.locked`), `advanced_threat_remediation._quarantine_file` (old XOR-0x5A), `_remediation_revert` (old code was a silent no-op — it scanned for a `*.quar` suffix no producer ever wrote), and `system_cleanup` restore (now hash-verified; `restored=2` marks evidence-lost instead of faking success). Legacy XOR sidecars stay restorable. 7 new tests — 146/146 pass.

**v29.42w**: Security hardening round 1 from the audit queue. Defender exclusions narrowed to data dirs only (`python.exe`/`.pyc`/`.pyd` exclusions removed, ASR rule restored to **Enabled** not AuditMode) — TASK-011 CRITICAL; PowerShell command-injection in WMI remediation fixed (class validation + quote-escaping) — TASK-012 CRITICAL; `config.json` now HMAC-signed with a DPAPI-protected key and tamper callbacks (hot-reload verifies every load; unsigned edits after first save are rejected and alerted) — TASK-014; KEV/EPSS lookups cached process-wide (no more per-file/per-process `VulnerabilityScanner` construction or linear catalog scans) — TASK-017; feed URL verification is HTTPS-only and the process allowlist skip is path-bound (TASK-013/015 partial). 139/139 tests.

**v29.42v**: Full-project **security architecture audit** (read-only — no code changed). Mapped the end-to-end attack surface (ingestion → detection → response → persistence) and produced a prioritized top-5 risk matrix: (1) Defender self-exclusion blind spot — global `python.exe` process + `.pyc`/`.pyd` extension exclusions and a user-writable-path exclusion, ASR rule left in AuditMode; (2) unverified/MITM-able threat-feed ingestion driving automated firewall blocks and auto-remediation (`VERIFICATION_HASHES` are placeholder strings); (3) PowerShell command-injection from threat-name interpolation in remediation; (4) no privilege segregation — parsers/YARA/sklearn share the elevated token; (5) tamperability of config/quarantine/DBs (`chmod` no-op on Windows). Includes detection-gap coverage (real-time WMI subscriptions, schtasks, service creation, DLL hijacking, hollowing, DoH/beaconing statistics), an evasion-resilience review with counter-heuristics, a false-positive stress test (3389 dual-listed as safe+RAT, capability-regex FPs), quarantine reversibility findings, and a 5-phase production-readiness roadmap. Full report: [`docs/SECURITY_AUDIT_2026-09-07.md`](docs/SECURITY_AUDIT_2026-09-07.md). Queue: TASK-011…TASK-018.
