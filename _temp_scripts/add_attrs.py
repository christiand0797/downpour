#!/usr/bin/env python3
"""
Add rain canvas enhancements to downpour_v29_titanium.py
"""
with open('downpour_v29_titanium.py', 'r') as f:
    lines = f.readlines()

# Find the line with "self._afterglow_phase = 0"
# Insert new attributes after it
new_attrs = '''
...
new_attrs_lines = new_attrs.split('\n')

for i, line in enumerate(lines):
    if 'self._afterglow_phase = 0' in line:
        # Insert after this line
        for j, new_line in enumerate(new_attrs.split('\n')):
            lines.insert(i + 1 + j, new_line + '\n')
        print('Inserted new attributes after _afterglow_phase')
        break

with open('downpour_v29_titanium.py', 'w') as f:
    f.writelines(lines)

print('Inserted new attributes')