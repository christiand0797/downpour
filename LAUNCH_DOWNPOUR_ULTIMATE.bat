...
title Downpour v29 Titanium -- ULTIMATE EDITION
chcp 65001 >nul 2>&1
color 0A

:: =============================================================================
::  DOWNPOUR v29 TITANIUM -- ULTIMATE EDITION LAUNCHER
::  Features: 
::   - Admin elevation with UAC prompt (single prompt)
::  - Persistent threat intelligence across launches (SQLite cache)
::  - Multi-source intel: OSINT, C2 feeds, darknet monitors, honeypots
::  - Background physics engine for rain animation (offloaded from main thread)
::  - Auto-defender/ASR exclusions with AttackSurfaceReductionOnlyExclusions
::  - C2/botnet firewall blocks (Kimwolf, BadBox2, Mozi, AISURU, CobaltStrike)
::  - Background physics engine for rain (60fps offloaded from main thread)
::  - Thread-safe canvas command queue + DB operation runner
::  - Persistent intel cache across launches (SQLite + JSON)
::  - Darknet/OSINT monitors: Tor hidden services, pastebins, malware repos
::  - Auto-updating KEV/EPSS/CISA feeds with local caching
::  - Log rotation (last 3), temp cleanup, RAM check
::  - ASR 3b576869 toggle during pip, restored to AuditMode after
::  - Persistent intel cache across launches (SQLite + JSON)
::  - Darknet/OSINT monitors: Tor hidden services, pastebins, malware repos
::  - Auto-updating KEV/EPSS/CISA feeds with local caching
::  - Persistent process list cache for faster startup
::  - Smart threat actor tracking with MITRE ATT&CK mapping
::  - Honeypot deployment for credential harvesting detection
:: =============================================================================
...
"!PY!" -c "import psutil, requests, cryptography, watchdog, nvidia_ml_py, colorama, wmi, pywin32, sklearn, yara, PIL, dns, netifaces, joblib, tqdm, pyperclip, dateutil, numpy, scipy, scapy, psycopg2, redis" >nul 2>&1
if !errorlevel! neq 0 (
    echo   [..] Installing missing dependencies...
    "!PY!" -m pip install --upgrade pip --quiet --break-system-packages --no-warn-script-location >nul 2>&1
    for %%P in (psutil requests cryptography watchdog nvidia-ml-py colorama wmi pywin32 scikit-learn yara-python pillow dnspython netifaces joblib tqdm pyperclip dateutil numpy scipy scapy psycopg2 redis redis-py) do (
...
netsh advfirewall firewall add rule name=DOWNPOUR_C2_KIMWOLF dir=out action=block remoteip=93.95.112.50,93.95.112.51,93.95.112.52,93.95.112.53,93.95.112.54,93.95.112.55,93.95.112.56,93.95.112.57,93.95.112.57,93.95.112.58,93.95.112.58,93.95.112.59,85.234.91.247 enable=yes >nul 2>&1
netsh advfirewall firewall add rule name=DOWNPOUR_C2_KIMWOLF_IN dir=in action=block remoteip=93.95.112.50,93.95.112.51,93.95.112.52,93.95.112.53,93.95.112.53,93.95.112.54,93.95.112.54,93.95.112.55,93.95.112.56,93.95.112.56,93.95.112.57,93.95.112.57,93.95.112.58,93.95.112.58,93.95.112.59,85.234.91.247 enable=yes >nul 2>&1
...
echo  =============================================================================
echo.
echo   Launching Downpour v29 Titanium -- ULTIMATE EDITION...
